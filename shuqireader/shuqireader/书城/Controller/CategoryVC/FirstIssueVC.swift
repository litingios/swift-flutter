...
class FirstIssueVC: LTSuperViewController {
    
    private var slidesList: [SlidesModel]? // 穿插的广告数据
    private var booksList: [BigModel]?
    ...
    /// 获取轮播图数据
    func loadData() -> Void {
        let path = Bundle.main.path(forResource: "home_ firstIssue", ofType: "json")
...
//                    self.booksList = self.booksList! + arr as? [BigModel]
//                }else{
...
extension FirstIssueVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

    func numberOfSections(in collectionView: UICollectionView) -> Int {
        return (self.booksList?.count ?? 0)-1
    }
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if (kind == UICollectionView.elementKindSectionHeader) {
            ...
            if indexPath.section == 0 {
                ...
                headerImg.titleStr = "全网首发 女生必读"
                headerImg.slidesList = self.slidesList
                headerImg.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: 185)
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        if section == 0 {
            return CGSize(width: ScreenWidth, height: 185)
        }else{
